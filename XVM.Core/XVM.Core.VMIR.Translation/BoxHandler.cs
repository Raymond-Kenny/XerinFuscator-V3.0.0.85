...
{
	public class BoxHandler : GInterface0
	{
		...
		public IIROperand Translate(ILASTExpression ilastexpression_0, IRTranslator irtranslator_0)
		{
			...
			TypeDef typeDef = ((ITypeDefOrRef)ilastexpression_0.Operand).ResolveTypeDef();
			...
			int bOX = irtranslator_0.VM.Runtime.VMCall.BOX;
			irtranslator_0.JqX().Add(new IRInstruction(IROpCode.PUSH, iIROperand));
			irtranslator_0.JqX().Add(new IRInstruction(IROpCode.VCALL, IRConstant.FromI4(bOX), IRConstant.FromI4(id)));
